...
//@route     DELETE/experts/:expert_is/delete
//@access    private(admin only)
router.get("/:expert_id/edit", isLoggedIn, authorize('admin'), function (req, res) {
    Expert.findById(req.params.expert_id, function (err, expert) {
        if (err) {
            res.redirect("back");
        } else {
           res.render('experts/editexpert',{expert})
        }
    });
});


router.put("/:expert_id", isLoggedIn, authorize('admin'), function (req, res) {
    Expert.findByIdAndUpdate(req.params.expert_id,req.body.expert, function (err, expert) {
        if (err) {
            res.redirect("back");
        } else {
            res.redirect('/experts/showexpert')
